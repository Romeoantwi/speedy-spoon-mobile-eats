...
const Cart = ({ items, isOpen, onClose, onUpdateQuantity, onClearCart, total, deliveryFee }: CartProps) => {
  ...
  const handleAddressSubmit = async (address: string, allergies?: string) => {
    ...
    try {
      ...
      // Store order data for after payment including notes
      const finalNotes = [allergies, orderNotes].filter(Boolean).join('\n');
      const orderData = { items, address, allergies: finalNotes };
      ...
      if (result?.status === 'success') {
        ...
        if (orderId) {
          onClearCart();
          setPendingOrderData(null);
          setOrderNotes(''); // Clear order notes after successful order
          toast({
      ...
      <div className="bg-white w-full h-full md:h-auto md:max-h-[80vh] md:w-96 md:rounded-lg overflow-hidden flex flex-col">
        ...
        {!isProcessingOrder && !paymentError && !currentOrder && !showOrderHistory && items.length > 0 && !showAddressForm && (
          <div className="p-4 border-t sticky bottom-0 bg-white z-10">
            ...
            {/* Order Notes Section */}
            <div className="mb-4">
              <label htmlFor="order-notes" className="block text-sm font-medium text-gray-800 mb-2">
            ...
            <div className="flex justify-between items-center text-lg font-bold text-gray-900 mb-4">
              ...
              <p className="text-orange-600">{formatCurrency(totalWithDelivery)}</p>
            </div>
